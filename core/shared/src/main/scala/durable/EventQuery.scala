 ...
 * The trait defines receive and | in terms of toBuilder, which each subtype implements.
 */
sealed trait SingleEventQuery[A]:
  /** Convert to a CombinedBuilder with a single condition */
  def toBuilder[S <: DurableStorageBackend](using backend: S, storage: DurableStorage[A, S]): CombinedBuilder[A, S]

  /** Create Durable that suspends until this condition is met */
  ...
  def |[B, S <: DurableStorageBackend](other: SingleEventQuery[B])(using
    backend: S,
    selfStorage: DurableStorage[A, S],
    otherStorage: DurableStorage[B, S]
  ): CombinedBuilder[A | B, S] =
    toBuilder(using backend, selfStorage) | other

/**
 * Single event query - wait for a specific event type.
 */
case class SingleEvent[E](eventName: String) extends SingleEventQuery[E]:
  def toBuilder[S <: DurableStorageBackend](using backend: S, storage: DurableStorage[E, S]): CombinedBuilder[E, S] =
    CombinedBuilder[E, S](
...
case class TimerDuration(duration: FiniteDuration) extends SingleEventQuery[TimeReached]:
  def toBuilder[S <: DurableStorageBackend](using backend: S, storage: DurableStorage[TimeReached, S]): CombinedBuilder[TimeReached, S] =
    val wakeAt = Instant.now().plusMillis(duration.toMillis)
    CombinedBuilder[TimeReached, S](
      events = Map.empty,
...
case class WorkflowCompletion[R](targetId: WorkflowId) extends SingleEventQuery[WorkflowResult[R]]:
  def toBuilder[S <: DurableStorageBackend](using backend: S, storage: DurableStorage[WorkflowResult[R], S]): CombinedBuilder[WorkflowResult[R], S] =
    CombinedBuilder[WorkflowResult[R], S](
      events = Map.empty,
      timerAt = None,
